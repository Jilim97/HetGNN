...
if __name__=='__main__':
    ...
    parser.add_argument('--emb_method', type=str, default='embalpha', help='Embedding method')
    parser.add_argument('--seed', type=int, default=42, help='Random Seed')
    parser.add_argument('--exp_name', type=str, default='DLPWhole-LinEmb', help='Experiment Name')
    ...
    transform_traintest_gene = T.RandomLinkSplit(num_val=args.val_ratio,
                                            num_test=args.test_ratio,
                                            disjoint_train_ratio=args.disjoint_train_ratio,
                                            neg_sampling_ratio=args.npr,
                                            add_negative_train_samples=args.npr,
                                            edge_types=('gene', 'interacts_with', 'gene'),
                                            rev_edge_types=('gene', 'rev_interacts_with', 'gene'),
                                            is_undirected=True)

    train_data_gene, val_data_gene, test_data_gene = transform_traintest_gene(heterodata_obj)
    
    # train_data_cell = to_homo(train_data_cell)
    # train_data_gene = to_homo(train_data_gene)
    # val_data_cell = to_homo(val_data_cell)
    # val_data_gene = to_homo(val_data_gene)
    # test_data_cell = to_homo(test_data_cell)
    # test_data_gene = to_homo(test_data_gene)
    # print(train_data_cell)
    ...
    for epoch in range(n_epochs):
        ...
        with torch.no_grad():
            if args.val_ratio != 0.0:
                ...
                index = val_data.edge_label_index
                msk = index[1] > (gene_num-1)

                ground_truth = val_data.edge_label
                val_loss = loss_fn(out[msk], ground_truth[msk])

                auc_val = roc_auc_score(ground_truth[msk].cpu(), pred[msk].cpu())
                ap_val = average_precision_score(ground_truth[msk].cpu(), pred[msk].cpu())
                ap_val_all = average_precision_score(ground_truth.cpu(), pred.cpu())
        ...
        if args.log:
            if args.val_ratio != 0.0:
                run.log({'epoch': epoch, 'train loss': total_train_loss/len(train_loader),
                        'val loss': val_loss, 'val auc': auc_val, 'val ap': ap_val, 'Full val ap': ap_val_all,
    ...
    path = BASE_PATH + f'DLP/Whole/Model/{args.exp_name}-{args.cell_feat}-{args.seed}-{final_epoch}.pt'
    ...
    if args.test_ratio != 0.0:
        ...
        ap_test_all = average_precision_score(ground_truth, pred)


        # pred = torch.sigmoid(out)
        # ground_truth = test_data.edge_label

        # index = test_data.edge_label_index
        # msk = index[1] > (gene_num-1)

        # ap_test = average_precision_score(ground_truth[msk].detach().cpu(), pred[msk].detach().cpu())
        # ap_test_all = average_precision_score(ground_truth.detach().cpu(), pred.detach().cpu())


        # edge_dict = {(int(index[0][msk][i]), int(index[1][msk][i])): i for i in range(len(index[0][msk]))}
        # test_probs = np.ones((len(whole_cells)*len(whole_genes)), dtype=float) * 3
        # test_labels = np.ones((len(whole_cells)*len(whole_genes)), dtype=float) * 3

        # for i in range(len(whole_edges[0])):
        #     gene_id = int(whole_edges[0][i])
        #     cell_id = int(whole_edges[1][i])

        #     if edge_dict.get((gene_id, cell_id)):
        #         label_idx = edge_dict[(gene_id, cell_id)]
        #         test_probs[i] = pred[msk][label_idx]
        #         test_labels[i] = ground_truth[msk][label_idx]

        # test_pred_df = construct_complete_predMatrix(total_predictions=test_probs,
        #                                                 edge_index=whole_edges,
        #                                                 index=whole_cells,
        #                                                 columns=whole_genes)

        # test_labels_df = construct_complete_predMatrix(total_predictions=test_labels,
        #                                                 edge_index=whole_edges,
        #                                                 index=whole_cells,
        #                                                 columns=whole_genes)
                                   
        # test_gene_ap, test_assay_ap = [], []
        # for i, row in test_pred_df.iterrows():
        #     assay_msk = row.values != 3
        #     if True in assay_msk:
        #         test_assay_ap.append(average_precision_score(y_true=test_labels_df.loc[i][assay_msk],
        #                                             y_score=row.values[assay_msk]))

        # for col in test_pred_df.columns:
        #     gene_msk = test_pred_df[col].values != 3
        #     if True in gene_msk:
        #         test_gene_ap.append(average_precision_score(y_true=test_labels_df[col].values[gene_msk],
        #                                             y_score=test_pred_df[col].values[gene_msk]))

        run.log({"test AP": ap_test, "Full test AP": ap_test_all, "test gene AP": np.mean(test_gene_ap), "test assay AP": np.mean(test_assay_ap),})
    ...
    gene_embs_df.to_csv(BASE_PATH+f"DLP/Whole/File/"\
    ...
    cell_embs_df.to_csv(BASE_PATH+f"DLP/Whole/File/"\
    ...
    if args.drugs:
        ...
        drug_embs_df.to_csv(BASE_PATH+f"DLP/Whole/File/{args.cell_feat}/"\
    ...
    if args.save_full_pred:
        ...
        tot_pred_deps.to_csv(BASE_PATH+f"DLP/Whole/File/"\
